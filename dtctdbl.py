...
def detect():
    if run:
        ...
        if sharpenOn is False and nightVis is False:
            ...
            screens = cv2.cvtColor(screens, cv2.COLOR_BGRA2BGR) # Perform well on this color with mss

            # predict frame
            result = model(screens, iou=ioudef, conf=confdef, imgsz=acudef)
            annotated_frame = cv2.cvtColor(result[0].plot(), cv2.COLOR_BGRA2RGB)
            annotated_frame = cv2.resize(annotated_frame, (screen_width, hl))
            ...
            listwidget[7].config(text=f"{statusUpdate}")
            screens = np.array(img)
            screens = cv2.cvtColor(screens, cv2.COLOR_BGRA2BGR)
...
label = Label(wdw, width=screen_width-10, height=hl-25, borderwidth=5,
